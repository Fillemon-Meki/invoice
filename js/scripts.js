...
// Safe scroll function to prevent undefined offset errors
function safeScrollToElement(selector) {
		...
		if (errorCounter > 0) {
			...
			$.ajax({
				...
				error: function(xhr, status, error){
					var errorMessage = "<strong>Error " + xhr.status + "</strong>: ";
					try {
						...
						if(xhr.responseText) {
							...
							errorMessage += error || status || "Unable to create invoice. Please check your data and try again.";
						}
					}
					...
					safeScrollToElement('#response');
					$btn.button("reset");
					console.error('Invoice creation error:', xhr.status, error, xhr.responseText);
				}
	...
	function actionLogin() {
		...
		if (errorCounter > 0) {
			...
			jQuery.ajax({
				...
				error: function(xhr, status, error){
					var errorMessage = "<strong>Error</strong>: ";
					try {
						var response = JSON.parse(xhr.responseText);
						errorMessage += response.message || "Login failed. Please try again.";
					} catch(e) {
						if(xhr.responseText) {
							errorMessage += "Server error - " + xhr.responseText.substring(0, 150);
						} else {
							errorMessage += error || status || "Unable to login. Please check your credentials.";
						}
					}
					$("#response .message").html(errorMessage);
					$("#response").removeClass("alert-success").addClass("alert-warning").fadeIn();
