...
if ($action == 'create_invoice'){
	...
	$check_query = "SELECT COUNT(*) as count FROM invoices WHERE invoice = '".$invoice_number."'";
	...
	if($check_row['count'] > 0) {
		...
		echo json_encode(array(
			'status' => 'Error',
			'message' => 'Invoice number '.$invoice_number.' already exists. Please use a different invoice number.'
		));
		exit;
	}
	...
	if($mysqli -> multi_query($query)){
		...
		// Generate PDF BEFORE sending JSON response
		...
		// Log if PDF was created successfully
		if(file_exists($pdf_path)) {
			error_log('Invoice PDF created successfully: ' . $pdf_path);
		} else {
			error_log('Invoice PDF NOT created - file does not exist: ' . $pdf_path);
		}
		
		} catch(Exception $e) {
			error_log('Invoice PDF generation error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
			// PDF generation error - still success since data was saved
		} catch(Error $e) {
			error_log('Invoice PDF generation fatal error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
			// Fatal error during PDF - still success since data was saved
		}
		...
		// Log any output that was captured
		if(!empty($buffer_content)) {
			error_log('PDF generation output/errors: ' . substr($buffer_content, 0, 500));
		}
		
		// NOW send JSON response and exit
		header('Content-Type: application/json');
		echo json_encode(array(
			'status' => 'Success',
		...
		error_log('Invoice creation error: ' . $error_message);
		echo json_encode(array(
			'status' => 'Error',
...
if($action == 'login') {
    ...
    if($count > 0) {
		...
		// Ensure row was fetched successfully
		if($row && isset($row['username'])) {
			...
			echo json_encode(array(
				'status' => 'Error',
				'message' => 'Login error: Unable to retrieve user data!'
			));
		}
    } else {
    	echo json_encode(array(
